...
int kisiSorgula(kisiBilgileri *uyeListesi,int ID){
	...
	int i;
		...
		if(uyeListesi[i].kutuphaneID==ID){
			return ID; //bu ID daha once kullanýldý.
		}
	}
...
int kayitOl(kisiBilgileri *uyeListesi){
	...
	char kisiAdi[20],kisiSoyadi[20];
	char ch;
	srand(time(NULL));// ýd farklýlýgý için
	for(i=0; i<MAX_KISI_SAYISI; i++){
...
int girisYap(kisiBilgileri *uyeListesi){
	...
	int ID,sifre,i;
	printf("Lutfen kutuphene ID bilginizi giriniz:");
	scanf("%d",&ID);
	...
	for(i=0; i<MAX_KISI_SAYISI; i++){
		...
		if(uyeListesi[i].kutuphaneID==ID){
			...
			return -1; //sifre yanlis girildi ; þifre baþarýsýz.
		}
	}
	return -2;// 0 ID degerine ait kimse bulunamadi.
}
...
void kitaplariYedekle(kitapBilgileri *kitaplik){
	...
	if(kitapKayitlari==NULL){
		fprintf(stderr,"Kitap listesine ulasilirken ");
		exit(1);
	}
...
int main(void){
 ...
 uyeListesi=(kisiBilgileri*)calloc(MAX_KITAP_SAYISI,sizeof(kisiBilgileri));
	...
	if(ch=='K' ||ch=='k'){
		...
		girisDurumu=kayitOl(uyeListesi);// ogrenci mi calisan mi
	}
	
	else if(ch=='G'||ch=='g'){
		girisDurumu=girisYap(uyeListesi);// ogrenci mi calisan mi
	    if(girisDurumu==-1){
