...
/*
author: Emirhan Azgün

https://1000kitap.com/kukreyenirmak
...
typedef struct{
	...
	int kisiSayisi;
	int odaDurumu;// doluysa 1 boþsa 0 olacak
...
void rezervasyonTemizle(odaBilgileri *odalar,int odaNumarasi){
	...
	odalar[odaNumarasi-1].odaDurumu=0;//odayý boþalltýk
	free(odalar[odaNumarasi-1].musteri.musteriAdi);//müþteri adý ve soyadý için ayrýlan alanlar boþaltýldý.
	free(odalar[odaNumarasi-1].musteri.musteriSoyadi);

    printf("%d numarali odanin cikis/iptal islemi tamamlanmistir",odaNumarasi);
}
...
void doluOdaBilgileriYazdirma(odaBilgileri *odalar){
	...
	for(i=0; i<ODA_SAYISI;i++){
		...
		if(odalar[i].odaDurumu!=0){ //1 ise oda dolu, 0 ise boþ
			printf("---------------------------------------------------\n");
...
void odaBilgisiSorgulama(odaBilgileri *odalar,int odaNumarasi){
	...
	if(odalar[odaNumarasi-1].odaDurumu!=0){ // 0 dolu 1 boþ anlamýna geliyor.
...
void fiyatBilgisiYazdirma(){
	...
	toplamTutar=gunFarkiHesapla(girisTarihi,cikisTarihi)*kisiSayisi*150;// bir günlük bir kiþinin ücreti
...
void yedekKayitAlma(odaBilgileri *odalar){
	...
	for(i=0; i<ODA_SAYISI;i++){
		if(odalar[i].odaDurumu!=0){
			...
			fprintf(otelKayitlari,"Toplam Tutar:%.2lf TL\n",odalar[i].fiyatBilgisi);
		    fprintf(otelKayitlari,"Giris Tarihi:%d:%d:%d\n",odalar[i].girisTarihi.gun,odalar[i].girisTarihi.ay,odalar[i].girisTarihi.yil);
		    fprintf(otelKayitlari,"Cikis Tarihi:%d:%d:%d",odalar[i].cikisTarihi.gun,odalar[i].cikisTarihi.ay,odalar[i].cikisTarihi.yil);
...
int main(){
 ...
 if(belirlenenSifre==kullanicininGirdigiSifre){ // Kullanici otelin bir çalýþaný
